...
use super::super::turtle::scanline_fill_lines;
use super::{
...
fn geometry_of_group_children(
    ...
    use super::super::turtle::{DpiConvertingTurtle, GeometryCollectorTurtle, Terrarium};
...
impl<'a, T: Turtle> XmlVisitor for ConversionVisitor<'a, T> {
    ...
    fn visit_exit(&mut self, node: Node) {
        ...
        if node.tag_name().name() == GROUP_TAG_NAME
            && let Some(exiting) = self.layer_override_stack.pop()
        {
            ...
            if exiting.mode == Some(LayerMode::Fill) && !exiting.fill_segments.is_empty() {
